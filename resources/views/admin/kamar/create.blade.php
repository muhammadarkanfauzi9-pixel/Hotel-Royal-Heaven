...
@section('page_subtitle', 'Tambahkan kamar baru ke dalam sistem')

@section('content')
    <div class="bg-white rounded-lg shadow-md p-8 max-w-2xl">
        <h2 class="text-2xl font-bold text-gray-900 mb-8">Tambah Kamar Baru</h2>
        ...
        <form action="{{ route('admin.kamar.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="nomor_kamar" name="nomor_kamar" value="{{ old('nomor_kamar') }}" required placeholder="Contoh: 101, 102, 201" class="mt-1 w-full border-gray-300 rounded-md shadow-sm px-3 py-2 border focus:outline-none focus:ring-blue-500 focus:border-blue-500">
            </div>
            ...
            <div>
                ...
                <select id="id_tipe" name="id_tipe" required class="mt-1 w-full border-gray-300 rounded-md shadow-sm px-3 py-2 border focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    ...
                    @foreach($tipe as $t)
                        <option value="{{ $t->id_tipe }}" @if(old('id_tipe') == $t->id_tipe) selected @endif>
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4" placeholder="Masukkan deskripsi kamar..." class="mt-1 w-full border-gray-300 rounded-md shadow-sm px-3 py-2 border focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old('deskripsi') }}</textarea>
            </div>
            ...
            <div>
                ...
                <select id="status_ketersediaan" name="status_ketersediaan" required class="mt-1 w-full border-gray-300 rounded-md shadow-sm px-3 py-2 border focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    <option value="available" @if(old('status_ketersediaan') == 'available') selected @endif>Tersedia</option>
                    <option value="booked" @if(old('status_ketersediaan') == 'booked') selected @endif>Dipesan</option>
                    <option value="maintenance" @if(old('status_ketersediaan') == 'maintenance') selected @endif>Perbaikan</option>
                </select>
            </div>
